...
def load_contacts(filename):
    try:
        with open(filename, mode='r') as file:
            ...
            # Ensure all contacts have the required fields, defaulting to empty strings for new fields
            for contact in contacts:
                contact.setdefault('Name', '')
                contact.setdefault('Phone', '')
                contact.setdefault('Email', '')
                contact.setdefault('Address', '')
                contact.setdefault('Social Media', '')
            return contacts
...
def display_contacts(contacts):
    print("\n{:<20} {:<15} {:<30} {:<20} {:<20}".format("Name", "Phone", "Email", "Address", "Social Media"))
    print("-" * 105)
    for contact in contacts:
        print("{:<20} {:<15} {:<30} {:<20} {:<20}".format(
            contact.get('Name', ''), 
            contact.get('Phone', ''), 
            contact.get('Email', ''), 
            contact.get('Address', ''), 
            contact.get('Social Media', '')
        ))
